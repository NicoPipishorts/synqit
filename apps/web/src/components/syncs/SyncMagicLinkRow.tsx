...
type SyncMagicLinkRowProps = {
  magicLinkToken: string;
  magicLinkRevokedAt: string | null;
  className?: string;
  shareMode?: 'modal' | 'inline' | 'none';
};

const getPublicSyncPath = (magicLinkToken: string): string => `/sync/${magicLinkToken}`;

const getPublicSyncUrl = (magicLinkToken: string): string => {
  if (typeof window === 'undefined') {
    return getPublicSyncPath(magicLinkToken);
  }

  return `${window.location.origin}${getPublicSyncPath(magicLinkToken)}`;
};

export const SyncMagicLinkRow = ({
  magicLinkToken,
  magicLinkRevokedAt,
  className,
  shareMode = 'modal',
}: SyncMagicLinkRowProps) => {
  const { t } = useI18n();
  const { showToast } = useToast();
  const [isShareSheetOpen, setIsShareSheetOpen] = useState(false);

  const syncUrl = getPublicSyncUrl(magicLinkToken);
  ...
  return (
    <div className={rootClassName}>
      <a
        href={getPublicSyncPath(magicLinkToken)}
        target="_blank"
        rel="noreferrer"
          ...
          >
            <div className="grid gap-5 pb-2 sm:gap-4 sm:pb-1">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 pr-2 sm:grid sm:grid-flow-col sm:auto-cols-fr sm:gap-4 sm:overflow-visible sm:pb-1 sm:pr-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <CTAButton
                  className="h-auto min-w-20 shrink-0 flex-col gap-1.5 rounded-2xl !border-0 !bg-transparent !px-1 !py-0 text-[11px] font-bold text-app-text !shadow-none hover:!bg-transparent hover:text-brand-pink hover:!translate-y-0 sm:min-w-19 sm:gap-2"
                  onClick={() => {
                    void copyMagicLink();
                    setIsShareSheetOpen(false);
                  }}
